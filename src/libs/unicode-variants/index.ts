import { setToPattern, arrayToPattern, escape_regex, sequencePattern } from './regex';
import { allSubstrings } from './strings';
...
export const g_code_points: TCodePoints = [[0, 65535]];

const accent_pat = '[\u0300-\u036F\u{b7}\u{2be}\u{2bc}]';

export let g_unicode_map: TUnicodeMap;
...
export const initialize = (_code_points?: TCodePoints) => {
  if (g_unicode_map !== undefined) return;
  g_unicode_map = generateMap(_code_points || g_code_points);
};
...
export const mapSequence = (strings: string[], min_replacement: number = 1): string => {
  ...
  strings = strings.map((str) => {
    if (g_unicode_map[str]) {
      chars_replaced += str.length;
    }
    return g_unicode_map[str] || str;
  });
